...
const Styles = styled.div<{ height: number; width: number }>`
  padding: ${({ theme }: { theme: any }) =>
    (theme as ExtendedTheme).sizeUnit * 2}px;
  ...
  .ant-checkbox-wrapper {
    color: ${({ theme }: { theme: any }) =>
      (theme as ExtendedTheme)?.colorText || 'inherit'} !important;
  }
  ...
  .ant-select:not(.ant-select-customize-input) .ant-select-selector {
    background-color: ${({ theme }: { theme: any }) =>
      (theme as ExtendedTheme)?.colorBgContainer || 'transparent'} !important;
    border-color: ${({ theme }: { theme: any }) =>
      (theme as ExtendedTheme)?.colorBorder || '#d9d9d9'} !important;
    color: ${({ theme }: { theme: any }) =>
      (theme as ExtendedTheme)?.colorText || 'inherit'} !important;
  }
...
export default function SupersetPluginChartCustomControls(
