...
export default function CustomDatePicker(
  props: CustomDatePickerTransformedProps,
) {
  const {
    height,
    width,
  ...
  const emitSingleFilter = React.useCallback(
    (dateStr: string | null) => {
  ...
  const emitRangeFilter = React.useCallback(
  ...
  const presetChoices = useMemo(() => {
    ...
    return [
      ...
      {
        label: 'Last 7 Days',
        value: [dayjs().subtract(7, 'd'), dayjs()] as [Dayjs, Dayjs],
      },
      {
        label: 'Last 30 Days',
        value: [dayjs().subtract(30, 'd'), dayjs()] as [Dayjs, Dayjs],
      },
      {
        label: 'This Month',
  ...
  const bgColor =
    (theme as any)?.colorBgContainer ||
    (theme as any)?.colors?.grayscale?.dark1 ||
    '#ffffff';
  const isDarkMode = (() => {
